...
template <class T>
pchip_t<T>::pchip_t(pchip_t&& a_other)
{
  std::swap(m_nodes_count, a_other.m_nodes_count);
  std::swap(m_x, a_other.m_x);
  std::swap(m_y, a_other.m_y);
  std::swap(m_derivatives, a_other.m_derivatives);
  std::swap(m_c2, a_other.m_c2);
  std::swap(m_c3, a_other.m_c3);
}
